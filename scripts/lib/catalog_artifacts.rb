...
module CatalogArtifacts
  ...
  SITEMAP_OUTPUT = File.join(ROOT, "sitemap.xml")
  ROBOTS_OUTPUT = File.join(ROOT, "robots.txt")
  FEED_OUTPUT = File.join(ROOT, "feed.xml")
  MANIFEST_OUTPUT = File.join(ROOT, "site.webmanifest")
  ...
  # Generates the in-<head> discovery block. The schema describes the atlas and
  # its machine-readable export without embedding hundreds of duplicate nodes.
  def seo_block(payload = site_payload)
    ...
    image = "#{url}assets/awesome-egocentric-social-card.png"
    hf_csv = "#{meta.fetch('huggingface_url').chomp('/')}/resolve/main/awesome-egocentric-atlas.csv"
    site_json_url = "#{url}site-data.json"
    audit_date = meta.fetch("last_major_audit")
    keywords = [
      "egocentric AI", "egocentric vision", "first-person video", "embodied AI",
      "robotics", "vision-language-action", "world models", "egocentric memory",
      "AR/VR", "hand-object interaction"
    ]
      ...
      "@graph" => [
        {
          ...
          "potentialAction" => {
            ...
            "target" => {
              "@type" => "EntryPoint",
              "urlTemplate" => "#{url}?q={search_term_string}#catalog"
            },
        ...
        },
        {
          ...
          "mainEntity" => { "@id" => "#{url}#catalog-export" },
          "additionalProperty" => [
            {
              "@type" => "PropertyValue",
              "name" => "Egocentric resources",
              "value" => summary_data.fetch("egocentric_resources")
            },
            {
              "@type" => "PropertyValue",
              "name" => "Adjacent resources",
              "value" => summary_data.fetch("adjacent_resources")
            }
          ]
        },
        {
          "@type" => "Dataset",
          "@id" => "#{url}#catalog-export",
          "name" => "#{title} catalog export",
          "description" => "Machine-readable catalog of the resources curated by #{title}.",
          "url" => meta.fetch("huggingface_url"),
          "dateModified" => audit_date,
    ...
    meta_tags = [
      ...
      %(<link rel="alternate" type="application/atom+xml" title="#{html_escape(title)} updates" href="#{url}feed.xml">),
      %(<link rel="manifest" href="site.webmanifest">),
      %(<meta property="og:type" content="website">),
      %(<meta property="og:url" content="#{url}">),
      %(<meta property="og:site_name" content="#{html_escape(title)}">),
      ...
      %(<meta property="og:image:alt" content="#{html_escape(title)} — a curated atlas of egocentric AI resources">),
      %(<meta name="twitter:card" content="summary_large_image">),
      %(<meta name="twitter:title" content="#{html_escape(title)}">),
      %(<meta name="twitter:description" content="#{html_escape(desc)}">),
      %(<meta name="twitter:image" content="#{image}">),
      %(<meta name="twitter:image:alt" content="#{html_escape(title)} — a curated atlas of egocentric AI resources">),
  ...
  def sitemap_xml(payload = site_payload)
    url = payload.fetch("meta").fetch("pages_url")
    audit_date = payload.fetch("meta").fetch("last_major_audit")
    <<~XML
      <?xml version="1.0" encoding="UTF-8"?>
      <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        <url>
          <loc>#{html_escape(url)}</loc>
          <lastmod>#{audit_date}</lastmod>
  ...
  def robots_txt(payload = site_payload)
    url = payload.fetch("meta").fetch("pages_url")
    <<~TXT
      User-agent: *
      Allow: /
  ...
  def released_timestamp(entry, fallback)
    released = entry["released"].to_s
    return "#{released}-01T00:00:00Z" if released.match?(/\A\d{4}-\d{2}\z/)
    return "#{released}-01-01T00:00:00Z" if released.match?(/\A\d{4}\z/)

    fallback
  end

  def feed_xml(payload = site_payload)
    meta = payload.fetch("meta")
    url = meta.fetch("pages_url")
    updated = "#{meta.fetch('last_major_audit')}T00:00:00Z"
    entries = payload.fetch("resources")
      .sort_by { |entry| [entry["released"].to_s, entry["name"].to_s] }
      .reverse
      .first(30)
      .map do |entry|
        <<~ENTRY
          <entry>
            <title>#{html_escape(entry.fetch("name"))}</title>
            <id>#{html_escape(entry.fetch("url"))}</id>
            <link href="#{html_escape(entry.fetch("url"))}"/>
            <updated>#{released_timestamp(entry, updated)}</updated>
            <category term="#{html_escape(entry.fetch("kind"))}"/>
            <summary>#{html_escape(entry["scale"] || "New catalog resource")}</summary>
          </entry>
        ENTRY
      end.join
    ...
    <<~XML
      ...
      <feed xmlns="http://www.w3.org/2005/Atom">
        <title>#{html_escape(meta.fetch("title"))} updates</title>
        <id>#{html_escape(url)}feed.xml</id>
        <link href="#{html_escape(url)}feed.xml" rel="self"/>
        <link href="#{html_escape(url)}"/>
  ...
  def webmanifest(payload = site_payload)
    meta = payload.fetch("meta")
    JSON.pretty_generate(
      {
        "name" => meta.fetch("title"),
        "short_name" => "Ego Atlas",
        ...
        "icons" => [
          {
            "src" => "assets/awesome-egocentric-logo.png",
            "sizes" => "512x512",
            "type" => "image/png",
            "purpose" => "any"
          }
  ...
  def generated_files
    {
      ...
      SITEMAP_OUTPUT => sitemap_xml,
      ROBOTS_OUTPUT => robots_txt,
      FEED_OUTPUT => feed_xml,
      MANIFEST_OUTPUT => webmanifest,
